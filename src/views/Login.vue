...
<template>
  <div class="min-h-screen bg-background text-foreground flex flex-col items-center justify-center px-6">
    <div class="w-full max-w-md">
      <div class="mb-6 text-center">
        <h1 class="text-2xl font-bold tracking-tight">{{ t('admin.login.brandTitle') }}</h1>
        <p class="text-sm text-muted-foreground mt-1">{{ t('admin.login.brandSubtitle') }}</p>
        <p class="text-xs text-muted-foreground mt-2 px-4">{{ t('admin.login.brandDescription') }}</p>
      </div>
      ...
      <p class="mt-4 flex items-center justify-center gap-1 text-center text-xs text-muted-foreground">
        <span>© {{ new Date().getFullYear() }} Dujiao Management System ·</span>
        <a
          href="https://github.com/diyanqi/dujiao-admin"
          target="_blank"
        ...
        >
          ...
          <span>https://github.com/diyanqi/dujiao-admin</span>
        </a>
      </p>
    </div>
  </div>
